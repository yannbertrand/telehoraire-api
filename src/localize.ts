...
import type {
  ...
  XmltvLocalizedPerson,
  XmltvLocalizedProgramme,
} from '../types/xmltv.localized.ts';
import type { XmltvChannel, XmltvCredits, XmltvPerson, XmltvStringWithLang } from '../types/xmltv.i18n.ts';
...
function localizeChannels(channels: XmltvChannel[], lang: string): XmltvLocalizedChannel[] {
...
function localizeProgrammes(programmes: XmltvProgramme[], lang: string): XmltvLocalizedProgramme[] {
...
export function localizeCredits(credits: XmltvCredits): XmltvLocalizedCredits | undefined {
  ...
  if (credits.director && credits?.director?.length > 0) {
    localizedCredits.director = credits.director.map((director) => director._value);
  }
  ...
  if (credits.producer && credits?.producer?.length > 0) {
    localizedCredits.producer = credits.producer.map((producer) => producer._value);
  }
  if (credits.composer && credits?.composer?.length > 0) {
    localizedCredits.composer = credits.composer.map((composer) => composer._value);
  }
  ...
  if (credits.presenter && credits?.presenter?.length > 0) {
    localizedCredits.presenter = credits.presenter.map((presenter) => presenter._value);
  }
  if (credits.commentator && credits?.commentator?.length > 0) {
    localizedCredits.commentator = credits.commentator.map((commentator) => commentator._value);
  }
...
export function localizeUniqueXmltvStringWithLang(
  ...
  if (!Array.isArray(xmltvStringsWithLang) || xmltvStringsWithLang.length === 0) {
    return undefined;
  }
...
export function localizeXmltvStringsWithLang(xmltvStringsWithLang: XmltvStringWithLang[], lang: string): string[] {
  if (!Array.isArray(xmltvStringsWithLang) || xmltvStringsWithLang.length === 0) {
    return [];
  }
  ...
  return localizeXmltvStringsWithLang(xmltvStringsWithLang, xmltvStringsWithLang[0].lang);
}
