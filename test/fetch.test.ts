...
describe('#fetch', () => {
  ...
  describe('content-type warning', () => {
    it('should warn if the fetched XMLTV file has content-type looks not supported', async () => {
      ...
      vi.stubGlobal('fetch', () =>
        Promise.resolve({ ok: true, headers, text: () => Promise.resolve(expectedTextResult) }),
      );
      ...
      // then
      expect(warnSpy).toHaveBeenCalledWith("The fetched XMLTV file content-type doesn't look like XML.");
    ...
    it('should return the raw file text content anyway', async () => {
      ...
      // when
      const textResult = await fetchXmltvFile(new URL('https://example.org/file.txt'));
  ...
  it('should return the raw file text content', async () => {
    ...
    // when
    const textResult = await fetchXmltvFile(new URL('https://example.org/file.xml'));
