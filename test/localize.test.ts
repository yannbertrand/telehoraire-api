...
import { localize, localizeUniqueXmltvStringWithLang, localizeXmltvStringsWithLang } from '../src/localize.ts';
...
describe('#localize', () => {
  describe('sample', () => {
    let xmltv: Xmltv;
    beforeEach(async () => {
      const xmltvSampleText = await readFile(resolve('test', 'samples', 'fr_tnt_2024-09-19.xml'), 'utf8');
    ...
    it('should localize channels displayName', () => {
      ...
      // then
      const localizedChannelDisplayNames = result.channels.map((channel) => channel.displayName);
    ...
    it('should localize programmes title', () => {
      // when
      const result = localize({ programmes: getTonightPrime(xmltv.programmes, new Date('2024-09-19')) }, 'fr');

      // then
      const localizedProgrammeTitles = result.programmes.map((programme) => programme.title);
    ...
    it('should localize programmes subTitle', () => {
      ...
      // then
      const localizedProgrammeSubTitles = result.programmes.map((programme) => programme.subTitle);
      expect(localizedProgrammeSubTitles).toContain('Coccinella septempunctata');
    ...
    it('should localize programmes desc', () => {
      ...
      // then
      const localizedProgrammeDescs = result.programmes.map((programme) => programme.desc);
    ...
    it('should localize programmes category list', () => {
      ...
      // then
      const localizedProgrammeCategories = result.programmes.map((programme) => programme.category);
      expect(localizedProgrammeCategories).toContainEqual(['Série', 'Policier']);
      expect(localizedProgrammeCategories).toContainEqual(['Documentaire', 'Découvertes']);
      expect(localizedProgrammeCategories).toContainEqual(['Divertissement', 'Humour']);
    });
    ...
    it('should localize programmes credits list', () => {
      ...
      // then
      const localizedProgrammeCredits = result.programmes.map((programme) => programme.credits);
      expect(localizedProgrammeCredits).toContainEqual({
        ...
        editor: ['Hugh Howey'],
        producer: ['Fred Golan', 'Graham Yost', 'Rebecca Ferguson', 'Jonathan Midlane', 'Aric Avelino'],
      });
      expect(localizedProgrammeCredits).toContainEqual({ guest: ['Cyril Hanouna'] });
      expect(localizedProgrammeCredits).toContainEqual({
        actor: [
          'Nuno Lopes',
          'Wim Willaert',
  ...
  describe('credits', () => {
    ...
    it('should only localize credits director', () => {
      ...
      // then
      expect(result.programmes[0].credits?.director).toStrictEqual(['Réalisateur']);
    ...
    it('should localize all credits roles', () => {
        ...
        programmes: [
          {
            ...
            credits: {
              ...
              actor: [
                ...
                { _value: 'Acteur 2', url: [{ _value: 'https://example.org/', system: 'example.org' }] },
              ],
      ...
      // then
      expect(result.programmes[0].credits?.director).toStrictEqual(['Réalisateur']);
      expect(result.programmes[0].credits?.actor).toStrictEqual(['Acteur 1', 'Acteur 2']);
      expect(result.programmes[0].credits?.writer).toStrictEqual(['Scénariste']);
      expect(result.programmes[0].credits?.adapter).toStrictEqual(['Adapteur']);
      expect(result.programmes[0].credits?.producer).toStrictEqual(['Producteur']);
      expect(result.programmes[0].credits?.composer).toStrictEqual(['Compositeur']);
      expect(result.programmes[0].credits?.editor).toStrictEqual(['Éditeur']);
      expect(result.programmes[0].credits?.presenter).toStrictEqual(['Présentateur']);
      expect(result.programmes[0].credits?.commentator).toStrictEqual(['Commentateur']);
      expect(result.programmes[0].credits?.guest).toStrictEqual(['Invité 1', 'Invité 2', 'Invité 3']);
    });
  });
  ...
  describe('#localizeUniqueXmltvStringWithLang', () => {
    describe('when empty array', () => {
      it('should return undefined', () => {
        ...
        // when
        const result = localizeUniqueXmltvStringWithLang(xmltvStringsWithLang, 'fr');
